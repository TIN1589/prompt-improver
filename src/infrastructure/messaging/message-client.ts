...
export class MessageClient {
   ...
   */
  static async send<T = unknown>(message: ExtensionMessage, timeoutMs: number = 35000): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        if (!isSettled) {
          isSettled = true;
          reject(new Error('Extension runtime không phản hồi (Message Timeout).'));
        }
      }, timeoutMs);
