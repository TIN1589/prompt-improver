/**
 * scoringEngine.js — Đánh giá chất lượng Prompt đa chiều (Heuristics & Entropy-based Scoring)
 */

import { estimateTokens, extractEntities } from './utils.js';

export class PromptScoringEngine {
  /**
   * Đánh giá định lượng chất lượng của một prompt
   * @param {string} prompt
   * @returns {import('./types/prompt.js').PromptScore}
   */
  static evaluate(prompt) {
    ...
    // Trọng số: Clarity 40%, Context 40%, Conciseness 20%
   ...
   * @returns {{ original: import('./types/prompt.js').PromptScore, improved: import('./types/prompt.js').PromptScore, delta: number }}
   */
  static compare(originalPrompt, improvedPrompt) {
